...
#image.plot(mex_pre2[50:70,5:26,1,1])


mex_pre3<-mex_pre2[50:70,5:26,,] #encontrar el cuadro que representa la región
#mex_pre4<-mex_pre2[45:70,5:26,,]


#de aqui para abajo cambiar el 71 y el 46, por los valores que te salgan en dim(mex_pre3) 71,46
inicio_mapa<-array(NaN,dim=c(21,22))
fin_mapa<-array(NaN,dim=c(21,22))
duracion_mapa<-array(NaN,dim=c(21,22))
inicio_tendencia<-array(NaN,dim=c(21,22))
fin_tendencia<-array(NaN,dim=c(21,22))
duracion_tendencia<-array(NaN,dim=c(21,22))
Duracion2<-array(NaN,dim=c(21,22,123))
for(lati in 1:21){
	for(loni in 1:22){
...
col="black",ylab="DAYS",
main="Rainy Season Lenght (Days) in Southeast Mexico",
xlab="Year")

#image.plot(duracion_mapa)
mod2<- lm(durax[74:123]~c(74:123))
lines(anio,fitted(mod2),col=2,lwd=2)
text(x=min(anio)-1,
	y=min(durax[74:123])-2,
	label=expression(atop(R^2==0.01,P>0.58)),
	col="red",adj=c(0,0))
